...
<head>
    ...
    <style>
        ...
        .gradient-overlay {
            ...
            background: rgba(0,0,0,0.3); /* Gelapkan background sedikit */
            z-index: 1;
        }

        /* --- LAYER 2: FINANCE CARD ( Tengah ) --- */
        #overlay-finance {
        ...
        .clock-big {
            ...
            padding: 5px 35px; border-radius: 50px;
        ...
        .date-hijri { font-size: 1.6rem; font-weight: bold; color: #d4af37; } /* Warna Emas */
        .date-masehi { font-size: 1.3rem; }

        /* --- LAYER 4: JADWAL SHOLAT (Floating Bottom) --- */
        .schedule-bar {
            position: fixed; 
            bottom: 60px; /* Diangkat ke atas sedikit memberi ruang untuk running text */
        ...
        /* Active State (Kotak Oranye) */
        .schedule-item.active {
            background: linear-gradient(135deg, #f39c12, #d35400);
            color: #fff !important; border-radius: 15px;
            transform: scale(1.15); border: none; z-index: 101;
        ...
        /* --- LAYER 5: RUNNING TEXT (Footer - News Ticker Style) --- */
        .news-ticker {
            position: fixed; bottom: 0; left: 0; width: 100%;
            height: 50px;
            background: #111; /* Hitam Elegan */
            border-top: 3px solid #d4af37; /* Garis Emas Pemanis */
        ...
        .ticker-label {
            ...
            z-index: 2; box-shadow: 5px 0 15px rgba(0,0,0,0.5);
        }
        .ticker-content {
            flex: 1; color: #fff; font-size: 1.4rem; 
            font-family: 'Roboto', sans-serif;
            white-space: nowrap; overflow: hidden;
        }

        /* --- LAYER 99: CRITICAL OVERLAYS --- */
        #overlay-iqomah, #overlay-sholat {
            display: none; position: fixed; top: 0; left: 0; width: 100%; height: 100%;
...
<body>
    ...
    <div id="bg-container">
        ...
        <img id="bg-image" src="https://images.unsplash.com/photo-1564121211835-e88c852648ab?q=80&w=1920&auto=format&fit=crop" alt="Background">
    </div>

    <div id="overlay-finance">
        <div class="finance-card">
            <h2 class="finance-title"><i class="fas fa-coins"></i> LAPORAN KAS MASJID</h2>
            <div style="margin-bottom: 10px; color: #aaa;">Saldo Akhir Saat Ini</div>
            <div class="saldo-utama">Rp <span id="fin-saldo">0</span></div>
            <div class="finance-details">
                <div class="f-item">
                    <span class="f-label"><i class="fas fa-arrow-down text-in"></i> Pemasukan</span>
                    <span class="f-value text-in">Rp <span id="fin-masuk">0</span></span>
                </div>
                <div class="f-item">
                    <span class="f-label"><i class="fas fa-arrow-up text-out"></i> Pengeluaran</span>
                    <span class="f-value text-out">Rp <span id="fin-keluar">0</span></span>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="schedule-bar">
        <div class="schedule-item">
            <div class="schedule-label">Terbit</div>
            <div class="schedule-time" id="time-terbit">00:00</div>
        </div>
        <div class="schedule-item" id="box-subuh">
            <div class="schedule-label">Subuh</div>
            <div class="schedule-time" id="time-subuh">00:00</div>
            <div class="countdown-text" id="cd-subuh"></div>
        </div>
        <div class="schedule-item" id="box-dzuhur">
            <div class="schedule-label">Dzuhur</div>
            <div class="schedule-time" id="time-dzuhur">00:00</div>
            <div class="countdown-text" id="cd-dzuhur"></div>
        </div>
        <div class="schedule-item" id="box-ashar">
            <div class="schedule-label">Ashar</div>
            <div class="schedule-time" id="time-ashar">00:00</div>
            <div class="countdown-text" id="cd-ashar"></div>
        </div>
        <div class="schedule-item" id="box-maghrib">
            <div class="schedule-label">Maghrib</div>
            <div class="schedule-time" id="time-maghrib">00:00</div>
            <div class="countdown-text" id="cd-maghrib"></div>
        </div>
        <div class="schedule-item" id="box-isya">
            <div class="schedule-label">Isya</div>
            <div class="schedule-time" id="time-isya">00:00</div>
            <div class="countdown-text" id="cd-isya"></div>
        </div>
    </div>
    ...
    <div class="news-ticker">
        ...
        <div class="ticker-content">
            <marquee id="running-text" scrollamount="6">Selamat Datang di Masjid...</marquee>
        </div>
    </div>

    <div id="overlay-iqomah">
        <h1 style="font-size: 5rem;">WAKTU IQOMAH</h1>
        <div id="iqomah-timer" style="font-size: 12rem; font-weight: bold;">00:00</div>
        <p class="fs-2">Mohon Luruskan dan Rapatkan Shaf</p>
    </div>
    <div id="overlay-sholat"></div> <script>
            ...
            function nextSlide() {
                if(document.getElementById('overlay-iqomah').style.display === 'flex' || 
                   document.getElementById('overlay-sholat').style.display === 'flex') {
                    setTimeout(nextSlide, 5000); return;
                }
                ...
                if (sliderIndex < slidersData.length) {
                    ...
                    setTimeout(() => {
                        bgImage.src = slidersData[sliderIndex].image_url;
